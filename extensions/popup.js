...
document.getElementById('form').addEventListener('submit', async function(event) {
  ...
  const response = await fetch('/api/get_user?username=' + encodeURIComponent(username));
  const existing_user = await response.json();
  ...
  const registerResponse = await fetch('/api/register_user', {
      method: 'POST',
      headers: {
  ...
  // Hide the form and show the welcome screen or next steps
